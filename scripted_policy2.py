...
class PickAndTransferPolicy(BasePolicy):
    ...
    def generate_trajectory(self, ts_first):
        ...
        self.left_trajectory = [
            ...
            {"t": 100, "xyz": meet_xyz + np.array([-0.1, 0, -0.01]), "quat": meet_left_quat.elements, "gripper": 1}, # approach meet position
        ...
        self.right_trajectory = [
            ...
            {"t": 90, "xyz": box_xyz + np.array([0, 0, 0.23]), "quat": gripper_pick_quat.elements, "gripper": 1}, # approach the cube
            {"t": 130, "xyz": box_xyz + np.array([0, 0, 0]), "quat": gripper_pick_quat.elements, "gripper": 1}, # go down
            {"t": 170, "xyz": box_xyz + np.array([0, 0, 0]), "quat": gripper_pick_quat.elements, "gripper": 0}, # close gripper
            {"t": 200, "xyz": meet_xyz + np.array([0.05, 0, -0.02]), "quat": gripper_pick_quat.elements, "gripper": 0}, # approach meet position
            {"t": 220, "xyz": meet_xyz, "quat": gripper_pick_quat.elements, "gripper": -0.02}, # move to meet position
...
def test_policy(task_name, episode_num, onscreen_render):
    ...
    for episode_idx in range(episode_num):
        ...
        if onscreen_render:
            dpi = 100
            fig_width = (640 * 4) / dpi  # Total width for 3 cameras
            fig_height = 480 / dpi
            fig, axs = plt.subplots(1, 4, figsize=(fig_width, fig_height), dpi=dpi)  # 1 row, 3 columns for 3 cameras
            imgs = []
            for ax, cam_name in zip(axs, ['top', 'worms_eye', 'wrist_left', 'wrist_right']):  # Replace with your camera names
        ...
        for step in tqdm(range(episode_len)):
            ...
            episode.append(ts)
            if onscreen_render:
                for img, ax, cam_name in zip(imgs, axs, ['top', 'worms_eye', 'wrist_left', 'wrist_right']):
                    img.set_data(ts.observation['images'][cam_name])
                    ax.set_title(cam_name)  # This is op
                plt.pause(0.001)
        plt.close()
